...
class Admin extends CI_Model
{
	...
	public function factory_wise_factory_list($factoryid)
	{
		$query = "SELECT * FROM factory WHERE factoryid='$factoryid'";
		$result = $this->db->query($query);
		return $result->result_array();
	...
	public function date_wise_machine_transfer_list_id($pd, $wd)
	{
		...
		GROUP BY machine_renthistory_insert_id.rhid
	...
	public function machine_requisition_insert($data)
	{
		date_default_timezone_set('Asia/Dhaka');
		$data['idate'] = date("Y-m-d", strtotime($data['idate']));
		$data['fdate'] = date("Y-m-d", strtotime($data['fdate']));
		...
		$sql = "INSERT INTO machine_requisition_insert VALUES ('$data[ccid1]','$data[ccid]','$data[purpose]','$data[mcode]','$data[mtype]','$data[line]','$data[buyer]','$data[style]','$data[oqty]','$data[idate]','$data[plan]','$data[inhand]','$data[fdate]','$data[tdate]','$data[price]','0')";
		return $query = $this->db->query($sql);
	}
	...
	public function date_wise_machine_requisition_list($pd, $wd)
	{

		$pd = date("Y-m-d", strtotime($pd));
		$wd = date("Y-m-d", strtotime($wd));

		$query = "SELECT * FROM machine_requisition_insert_id
		
		WHERE mqrsdate BETWEEN '$pd' AND '$wd'
		
		ORDER BY mqrsdate DESC";

		$result = $this->db->query($query);
		return $result->result_array();
	}
	public function date_wise_factory_wise_machine_requisition_list($factoryid, $pd, $wd)
	{
		...
		WHERE factoryid='$factoryid' AND mqrsdate BETWEEN '$pd' AND '$wd'
	...
	public function machine_requisition_list_id($mriid)
	{
	...
	public function machine_requisition_list_details($mriid)
	{

		$query = "SELECT machine_requisition_insert_id.mriid,factoryid,mpurpose,mname,
				mtype,slname,buyerid,styleid,oqty,idate,planqty,inhandqty,
				fdate,tdate,rprice,COUNT(odid) AS odcount,
				DATEDIFF(tdate,fdate) AS stayingday,rremarks 
				FROM machine_requisition_insert_id

		JOIN machine_requisition_insert 
		ON machine_requisition_insert.mriid=machine_requisition_insert_id.mriid
		JOIN machine_purpose ON machine_purpose.mpid=machine_requisition_insert.mpid
		JOIN machine_name ON machine_name.mcode=machine_requisition_insert.mcode
		JOIN machine_type ON machine_type.mtid=machine_requisition_insert.mtid
		JOIN sewing_line_insert ON sewing_line_insert.slnid=machine_requisition_insert.slnid
		
		LEFT JOIN 
    	off_day_insert
    	ON offdate >=fdate AND offdate<=tdate AND factoryid=offfactoryid
		WHERE machine_requisition_insert_id.mriid='$mriid'
		
		GROUP BY machine_requisition_insert.mrid";
	...
	public function multiple_machine_rent_insert($data)
	{
		...
		$sql = "UPDATE machine_inventory SET cfactoryid='$data[dfid]',rentdays='$data[rd]',rentdate='$data[rentdate]',mistatus='8' WHERE minvid='$data[minvid]'";

		$sql1 = "INSERT INTO machine_renthistory_insert VALUES ('$data[ccid]','$data[rccid]','$data[minvid]','$data[macode]','$data[cfactoryid]','$data[dfid]','8','$data[rd]','$data[rp]','$data[rentdate]',CURTIME(),'$data[smonth]','$data[syear]','','','','','$data[remarks]','','','','','$data[rminvid]')";
	...
	public function multiple_machine_rent_return_insert($data)
	{
		...
		$sql = "UPDATE machine_inventory SET cfactoryid='$data[pfactoryid]',rentdays='0',rentdate='0000-00-00',mistatus='8' WHERE minvid='$data[minvid]'";

		$sql1 = "UPDATE machine_renthistory_insert SET mistatus='8',retrdate='$data[returndate]',retrtime=CURTIME(),remonth='$data[smonth]',reyear='$data[syear]',rentreturnremarks='$data[returnremarks]' WHERE minvid='$data[minvid]' AND mistatus='1'";
	...
	public function machine_intransit_list()
	{
		$query = "SELECT factoryname,pfactoryid,cfactoryid,
		mpurpose,(machine_name.mcode) AS mcode,
		minvid,rminvid,macode,mname,minfo,model,mtype,brandname,supplier,price,miqty,
	...
	public function factory_wise_machine_intransit_list($factoryid)
	{
		$query = "SELECT factoryname,pfactoryid,cfactoryid,
		mpurpose,(machine_name.mcode) AS mcode,
		minvid,rminvid,macode,mname,minfo,model,mtype,brandname,supplier,price,miqty,
		...
		JOIN machine_name ON machine_name.mcode=model_name.mcode
		JOIN machine_purpose ON machine_purpose.mpid=machine_name.mpid 
		WHERE mistatus='8' AND cfactoryid='$factoryid'
		ORDER BY factoryid,mpurpose,mname,minvid ASC";
		$result = $this->db->query($query);
		return $result->result_array();
	}
	public function multiple_machine_intransit_receive($data)
	{
		date_default_timezone_set('Asia/Dhaka');
		$data['receivedate'] = date("Y-m-d", strtotime($data['receivedate']));
		//$data['smonth'] = date('F', strtotime($data['returndate']));
		//$data['syear'] = date('Y', strtotime($data['returndate']));
		...
		// $ccid = $d1 . $t1;
		// $data['ccid'] = $ccid . $data['i'];
		if ($data["pfactoryid"] != $this->session->userdata('factoryid')) {
			$sql = "UPDATE machine_inventory SET mistatus='1' WHERE minvid='$data[minvid]'";

			$sql1 = "UPDATE machine_renthistory_insert SET mistatus='1',receivedate='$data[receivedate]',receivetime=CURTIME(),receiveremarks='$data[receiveremarks]' WHERE minvid='$data[minvid]' AND mistatus='8'";
			...
			$sql1 = "UPDATE machine_renthistory_insert SET mistatus='0',receivedate='$data[receivedate]',receivetime=CURTIME(),receiveremarks='$data[receiveremarks]' WHERE minvid='$data[minvid]' AND mistatus='8'";
	...
	public function year_wise_off_day_insert($data)
	{
		...
		$sql = "SELECT * FROM off_day_insert WHERE offdate='$data[offdate]' AND offfactoryid='$data[factoryid]'";
		$query = $this->db->query($sql);
		if ($query->num_rows() == 1) {

			$sql1 = "UPDATE off_day_insert SET offdate='$data[offdate]',offmonth='$data[smonth]',offyear='$data[syear]',sdate=CURDATE(),stime=CURTIME() 
			WHERE offfactoryid='$data[factoryid]' AND offdate='$data[offdate]'";
			$query1 = $this->db->query($sql1);
			//return false;
		} else {
	...
	public function machine_line_allocate_list($mpid, $cfactoryid)
	{
		...
		$query = "SELECT factoryname,pfactoryid,cfactoryid,
		mpurpose,(machine_name.mcode) AS mcode,
		(machine_inventory.minvid) AS minvid,(machine_inventory.rminvid) AS rminvid,
		(machine_inventory.macode) AS macode,
		mname,minfo,model,mtype,brandname,supplier,price,miqty,
		pdate,warranty,description,mistatus,rentdays,rentdate,slname,allocatedate
	...
	public function factory_wise_machine_rent_status($factoryid)
	{

		$query = "SELECT * FROM machine_inhouse_view
					WHERE  factoryid='$factoryid' AND totalmachinerent!='0'
					GROUP BY factoryid, 
					mpurpose,mname,
					mtype,
					mcode,
