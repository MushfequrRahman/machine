...
class Dashboard extends CI_Controller
{
	...
	public function show_machine()
	{
		...
		$output = '';
		foreach ($data as $row) {
			$output .= '<option value="' . $row["mcode"] . '">' . $row["mname"] . '</option>';
		}
	...
	public function machine_requisition_insert_form()
	{
		$this->load->database();
		$this->load->model('Admin');
		$data['title'] = 'Machine Requisition Insert';
		$this->load->view('admin/head', $data);
		$this->load->view('admin/toprightnav');
		$this->load->view('admin/leftmenu');
		$usertype = $this->session->userdata('user_type');
	...
	public function machine_requisition_insert()
	{
		date_default_timezone_set('Asia/Dhaka');
		$mprdate = $this->input->post('mprdate');
		$mprdate = date("Y-m-d", strtotime($mprdate));

		$mqrsdate = $this->input->post('mqrsdate');
		$mqrsdate = date("Y-m-d", strtotime($mqrsdate));

		$fmy = strtotime($mqrsdate);
		$month = date("F", $fmy);
		$year = date("Y", $fmy);
		$d = date('Y-m-d');
		$t = date("H:i:s");
		...
		$sql1 = "INSERT INTO machine_requisition_insert_id VALUES ('$ccid','$userid','$factoryid','$mqrsdate','$month','$year','$remarks','0')";
		...
		for ($i = 0; $i < count($purpose); $i++) {
			...
			$data["idate"] = $idate[$i];
			$data["plan"] = $plan[$i];
			$data["inhand"] = $inhand[$i];
			$data["fdate"] = $fdate[$i];
			$data["tdate"] = $tdate[$i];
			$data["price"] = $price[$i];

			$ins = $this->Admin->machine_requisition_insert($data);
		}
		if ($ins) {
			echo  "ok";
		} else {
	...
	public function date_wise_machine_requisition_list_form()
	{
		$this->load->database();
		$this->load->model('Admin');
		$data['title'] = 'Date Wise Machine Requisition List';
		$this->load->view('admin/head', $data);
		$this->load->view('admin/toprightnav');
		$this->load->view('admin/leftmenu');
		$this->load->view('admin/master_data/date_wise_machine_requisition_list_form', $data);
	}
	public function date_wise_machine_requisition_list()
	{
		$this->load->database();
		$this->load->model('Admin');
		$factoryid = $this->session->userdata('factoryid');
		...
		$data['wd'] = $wd;
		if ($usertype == '1') {
			//$data['ul'] = $this->Admin->date_wise_machine_requisition_list($pd, $wd);
			$data['ul'] = $this->Admin->date_wise_machine_requisition_list($pd, $wd);
			$this->load->view('admin/master_data/date_wise_machine_requisition_list', $data);
		}
		if ($usertype == '2') {
			//$data['ul'] = $this->Admin->date_wise_factory_wise_machine_requisition_list($factoryid, $pd, $wd);
			$data['ul'] = $this->Admin->date_wise_factory_wise_machine_requisition_list($factoryid, $pd, $wd);
			$this->load->view('admin/master_data/date_wise_machine_requisition_list', $data);
		}
		if ($usertype == '3') {
	...
	public function machine_requisition_list_details()
	{
		$this->load->database();
		$this->load->model('Admin');
		$data['title'] = 'Machine Requisition List Details';
		$mriid = $this->uri->segment(3);
		$this->load->view('admin/head', $data);
		$this->load->view('admin/toprightnav');
		$this->load->view('admin/leftmenu');
		$data['ul'] = $this->Admin->machine_requisition_list_details($mriid);
		$this->load->view('admin/master_data/machine_requisition_list_details', $data);
	}
	public function machine_requisition_print()
	{
		$this->load->database();
		$this->load->model('Admin');
		//$this->load->library('pdf');
		$data['title'] = 'Machine Rent Requisition';
		//$this->load->library('numbertowordconvertsconver');
		$factoryid = $this->session->userdata('factoryid');
		$mriid = $this->uri->segment(3);
		$this->load->view('admin/head', $data);
		$data['fl'] = $this->Admin->factory_wise_factory_list($factoryid);
		$data['ril'] = $this->Admin->machine_requisition_list_id($mriid);
		$data['rl'] = $this->Admin->factory_wise_machine_rent_status($factoryid);
		$data['ul'] = $this->Admin->machine_requisition_list_details($mriid);
		$this->load->view('admin/master_data/machine_requisition_print_html', $data);
	}
	...
	public function multiple_machine_intransit_form()
	{
		$this->load->database();
		$this->load->model('Admin');
		$data['title'] = 'Machine Rent Return Insert';
		...
		if ($usertype == '1') {
			$data['ul'] = $this->Admin->machine_intransit_list();
			$this->load->view('admin/master_data/multiple_machine_intransit_insert_form', $data);
		} elseif ($usertype == '2') {
	...
	public function multiple_machine_intransit_receive()
	{
		$this->load->database();
		$this->load->library('form_validation');
		$this->load->model('Admin');
		if ($this->input->post('submit')) {
			...
			$receiveremarks = $this->input->post('receiveremarks');
			for ($i = 0; $i < count($minvid); $i++) {
				$data["i"] = $i;
				//$data["macode"] = $macode[$i];
				$data["minvid"] = $minvid[$i];
				...
				$data["receiveremarks"] = $receiveremarks;
				// $data["rminvid"] = $rminvid[$i];
				$ins = $this->Admin->multiple_machine_intransit_receive($data);
			}

			if ($ins == TRUE) {
				$this->session->set_flashdata('Successfully', 'Successfully Recevied');
			} else {
				$this->session->set_flashdata('Un Successfully', 'Failed To Recevied');
			}
			redirect('Dashboard/machine_inventory_list', 'refresh');
		}
	}
